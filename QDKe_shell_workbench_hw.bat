...
set DJNLOG=0
if "x%DJNLOG%" == "x1" (
  bash --login -i -c "../env/env-workbench/workbench-hw-engine-search.sh lm2831" > %~dp0var/log/%~n0.log 2>&1
  %QDKT_UE% %~dp0var/log/%~n0.log
) else (
  bash --login -i -c "../env/env-workbench/workbench-hw-engine-search.sh lm2831"
)
:+++++++++++++++++++++++++++++++++++++
:: Doing Jobs Finish...
